---
title: "Del dato al modelo: pipelines personalizados utilizando python"
subtitle: "3er Encuentro Interdisciplinar: Gestión de datos en organizaciones"
...
-   Todo el código se encuentra disponible en {{< fa brands github size=1x >}} <a href="https://github.com/karbartolome/workshops" target="_blank">Repositorio</a> y en [Google Colab](https://drive.google.com/file/d/1LSxPtN9prP5aHnC_MNc7uWqeZI_mgd5d/view?usp=sharing)

# Caso: Transactions Fraud Detection (Kaggle)

## Prevención del fraude transaccional

::: {style="font-size: 70%;"}

Caso ilustrativo: se utiliza el dataset Transactions Fraud Detection por su valor didáctico y reproducibilidad. No representa necesariamente las variables ni modelos utilizados en sistemas reales.
...
Se cuenta con un dataset de `{python} df.shape[0]` transacciones (compras). Son `{python} df.shape[1]` variables generadas sintéticamente por lo que el foco está sobre cómo procesar los datos y no sobre la performance del modelo [^1].
...
[^1]: @calibracion2024 es un workshop similar a este, enfocado en la performance de modelos de clasificación binaria.
...
```{python}
#| echo: false
#| label: tbl-transformaciones-iniciales
#| tbl-cap: "Transformaciones iniciales"
...
@Devarakonda2023 propone un enfoque híbrido para la detección de fraude, combinando modelos supervisados y no supervisados (Issolation Forest). En este caso, se utilizará un score de cuán anómala es la transacción como variable explicativa del fraude.
...
```{python}
#| echo: false
Markdown(f"""La probabilidad de que la transacción sea fraudulenta es: {y_pred[0]:.2%} → ¿Aprobar o rechazar la transacción? Depende de la aversión al riesgo de la entidad en cuestión. 
""")
```
:::
