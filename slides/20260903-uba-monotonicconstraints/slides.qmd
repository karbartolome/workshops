...
format: 
    revealjs:
        ...
        footer: |
            <body>
            Cimbage - IADCOM | Facultad de Ciencias Económicas - UBA | <a href="https://karbartolome.github.io/workshops/" target="_blank" rel="noopener">Workshops</a>
            </body>
        self-contained: true
...
-   El contenido de estas diapositivas está disponible para ejecutar en [Google colab](https://drive.google.com/file/d/1S-CDufFiGUhzRMEM0LTWrUx_nmaSFl_O/view?usp=sharing)
...
Supuesto: dado el conocimiento de dominio de este tipo de datos, se espera que la probabilidad del evento `target` disminuya a mayor valor de la variable `feature` pero esa relación no sea estrictamente monotónica en los datos de la muestra. 


```{python}
#| label: fig-example-target-by-bin
#| fig-cap: "Tasa promedio de target por decil de feature"
#| warning: false
#| code-fold: true
#| code-summary: "Visualización de datos simulados"
df_target_by_bin = (
    df
    .assign(feature_bin=lambda data: pd.qcut(data["feature"], q=10))
    .groupby("feature_bin", as_index=False)
    .agg(avg_target=("target", "mean"), n=("target", "count"))
    .assign(feature_mid=lambda data: data["feature_bin"].apply(lambda x: x.mid))
)

fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(
    df_target_by_bin["feature_mid"],
    df_target_by_bin["avg_target"],
    marker="o",
    color=COLOR_BLUE,
    linewidth=2,
)
ax.set(
    title="Tasa promedio de target por decil de feature",
    xlabel="feature (punto medio del decil)",
    ylabel="Tasa promedio de target",
)
ax.grid(alpha=0.3)
plt.ylim(0, 1)
plt.tight_layout()
...
# Caso: Give me some credit (Kaggle)

## Datos

Give me some credit [@GiveMeSomeCredit] es un dataset con información financiera y demográfica de individuos. El objetivo es modelar el riesgo de incumplimiento a partir de esas características. Caso ilustrativo por su valor didáctico y reproducibilidad. No representa necesariamente las variables ni modelos utilizados en sistemas reales.
...
#| echo: true
#| eval: true
#| tbl-cap: "Métricas del modelo sin restricción (train vs. test)"
#| label: tbl-case-model-metrics
#| code-fold: true
#| code-summary: "Métricas del modelo sin restricción"
from utils.eval import compute_train_test_metrics
df_metrics = compute_train_test_metrics(
    modelo_sin_restriccion_credito, X_train, y_train, X_test, y_test
)

(GT(df_metrics, rowname_col="dataset")
    .fmt_number(columns=df_metrics.columns.drop("dataset").tolist(), decimals=3)
    .tab_options(
        column_labels_background_color=COLOR_TABLE_HEADER,
        table_font_names="Times New Roman"
    )
...
Supuesto: se espera que un mayor ratio de deuda no reduzca el riesgo de incumplimiento. Por eso se impone una restricción monotónica creciente sobre `DebtRatio`.
...
#| echo: true
#| eval: true
#| tbl-cap: "Métricas del modelo con restricción (train vs. test)"
#| label: tbl-case-model-constrained-metrics
#| code-fold: true
#| code-summary: "Métricas del modelo con restricción"
df_metrics = compute_train_test_metrics(
    modelo_con_restriccion_credito, X_train, y_train, X_test, y_test
)
...
#| echo: false
#| eval: true
#| tbl-cap: "Variación en la probabilidad al variar DebtRatio, manteniendo el resto de las variables constantes "
