...
Los modelos de *boosting* combinan múltiples árboles secuencialmente, donde cada nuevo árbol busca reducir los errores del modelo actual:

$$
\underbrace{\color{orange}{\widehat{y}_i^{(M)}}}_{\color{orange}{\text{predicción final}}}
=
\underbrace{\color{orange}{\widehat{y}_i^{(0)}}}_{\color{orange}{\text{predicción inicial}}}
+
\sum_{m=1}^{M}
\underbrace{\color{blue}{\eta}}_{\color{blue}{\text{tasa de aprendizaje}}}
\,
\underbrace{\color{red}{f_m(x_i)}}_{\color{red}{\text{árbol}}}
$$

Donde cada $\color{red}{f_m}$ es un árbol y $\color{blue}{\eta}$ controla cuánto contribuye cada nuevo árbol al modelo.

En cada iteración, se incorpora el árbol que minimiza la pérdida del modelo actualizado ^[Versión simplificada: en la práctica, este proceso se realiza mediante métodos basados en el gradiente de la función de pérdida.]:
...
Para garantizar monotonía respecto a la característica $x_j$, exigimos que la derivada parcial del modelo ensamblado conserve el signo deseado $c_j \in \{-1, 1\}$:

$$
c_j \cdot \frac{\partial \widehat{y}_i^{(M)}}{\partial x_j} \ge 0 \quad \forall i
$$

En cada iteración, el nuevo árbol no se busca sobre todo el espacio de árboles posibles, sino únicamente dentro del conjunto restringido $F_{\text{mono}}$ que respeta las reglas de corte monotónicas:
...
donde $F_{\text{mono}}$ es el espacio de árboles cuyos valores en las hojas cumplen $f(x_1) \le f(x_2)$ siempre que $x_{1, j} \le x_{2, j}$ (para una restricción creciente $c_j = +1$).
...
Se utilizan datos simulados para ilustrar el efecto de imponer restricciones monotónicas en un modelo de clasificación binaria. El dataset contiene `{python} n` observaciones y 2 columnas: `feature` y `target` (variable binaria: 0 o 1).
...
Se espera que la probabilidad del evento `target` disminuya a mayor valor de la variable `feature`, pero el quiebre local simulado hace que esa relación no sea estrictamente monotónica en los datos. Se ajusta un modelo de clasificación binaria con y sin restricción sobre la variable `feature`.

## Modelo con restricciones monotónicas

Se ajustan 2 modelos: uno sin restricción monotónica y otro con restricción monotónica decreciente sobre la variable `feature`:
...
- **Modelo sin restricción**: la probabilidad aumenta en ciertos valores (contraintuitivo, se espera que disminuya con la edad).
