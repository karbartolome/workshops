...
Si se asume que el modelo ordena bien, el mappeo de `scores` a probabilidades es `no decreciente`. De esta forma, este método genera una función escalonada no decreciente (monotónica creciente):
...
▪️ En ciertos casos de modelos de clasificación podría no importar la calibración de probabilidades (si interesa el ordenamiento)

▪️ Algunos modelos tienden a estar más calibrados por defecto (regresión logística)

▪️ Un modelo bien calibrado permite establecer puntos de corte diferenciales en función de la probabilidad predicha
